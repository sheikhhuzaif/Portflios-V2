...
def convert_goldparse(dataturks_JSON_FilePath):
    try:
        training_data = []
        lines = []
        with open(dataturks_JSON_FilePath, 'r', encoding="Latin 1") as f:
            lines = f.readlines()
...
def flat_accuracy(valid_tags, pred_tags):
    return np.array(pred_tags).mean()
...
def train_and_val_model(
    ...
    for _ in trange(epochs, desc="Epoch"):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            b_input_ids, b_input_mask, b_labels = batch[0]['input_ids'], batch[0]['attention_mask'], batch[0]['labels']
            ...
            preds_mask=preds_mask*1
            tr_batch_preds = np.argmax(tr_logits[0], axis=1)
            #print(tr_batch_preds)
        ...
        for batch in valid_dataloader:
            ...
            preds_mask = preds_mask.cpu().detach().numpy()
            preds_mask = preds_mask *1
            val_batch_preds = np.argmax(logits[0], axis=1)
        ...
        cl_report = classification_report([valid_tags], [pred_tags[:len(valid_tags)]])
        conf_mat = annot_confusion_matrix(valid_tags, pred_tags[:len(valid_tags)])
